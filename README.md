...
├── offline_eval.ipynb           # Colab: ScanNet setup + every model/format eval
...
Two different environments, one flat repo:

- **`offline_eval.ipynb`** — the ScanNet-benchmark workflow. Downloads the full `scannet_frames_25k` dataset fresh each run, builds the fixed test split, and evaluates YOLO26-Depth, DAv2, and OpenVINO checkpoints against it. Runs in Colab, where downloading a multi-GB dataset each time is normal.
- **`offline_depth_eval.py` / `offline_depth_eval_dav2.py` / `run_lab_eval.py` / `run_lab_eval_openvino.py`** — the lab-generalization workflow. No dataset download — these point at RealSense captures already sitting on your own machine and evaluate a checkpoint against those. Meant to run directly on your dev machine (`python3 run_lab_eval.py`), no cloud environment needed.
...
`run_lab_eval_openvino.py` is used for YOLO26-Depth checkpoints only. Its `preprocess()` method (plain 0-255 → 0-1 scaling with BGR→RGB conversion) matches Ultralytics' export convention.
